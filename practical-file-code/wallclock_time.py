...
class RedLight(object):
    ...
    def run(self):
        while True:
            print("time:", self.env.now, "red light", ", wall clock time: ", perf_counter())
            yield self.env.timeout(self.red_time)
            print("time:", self.env.now, "green light", ", wall clock time: ", perf_counter())
            yield self.env.timeout(self.green_time)
            print("time:", self.env.now, "yellow light", ", wall clock time: ", perf_counter())
