...
def run_theater(env, num_cashiers, num_servers, num_users):
    theater = Theater(env, num_cashiers, num_servers, num_users)
...
def main():
    # Setup
    random.seed(42)
    num_cashiers, num_servers, num_users = get_user_input()

    # Run the simulation
    env = simpy.Environment()
    env.process(run_theater(env, num_cashiers, num_servers, num_users))
    env.run(until=90)
